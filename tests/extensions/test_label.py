import json
import os
from pystac.extensions.label import LabelClasses, LabelCount, LabelOverview, LabelStatistics, LabelType
...
class LabelTest(unittest.TestCase):
    ...
    def test_from_file(self):
        ...
        overviews = get_opt(label_ext(label_example_1).label_overviews)
        ...
        overviews2 = get_opt(label_ext(label_example_2).label_overviews)
    ...
    def test_from_file_pre_081(self):
        ...
        self.assertEqual(len(label_ext(label_example_1).label_tasks or []), 2)
    ...
    def test_get_sources(self):
        ...
        for li in items:
            if label_ext(li).has_extension:
                sources = list(label_ext(li).get_sources() or [])
    ...
    def test_read_label_item_owns_asset(self):
        item = next(x for x in TestCases.test_case_2().get_all_items() if label_ext(x).has_extension)
    ...
    def test_label_description(self):
        ...
        label_desc = label_ext(label_item).label_description
        self.assertEqual(label_desc, label_item.properties['label:description'])

        # Set
        label_ext(label_item).label_description = "A detailed description"
    ...
    def test_label_type(self):
        ...
        label_type = label_ext(label_item).label_type
        self.assertEqual(label_type, label_item.properties['label:type'])

        # Set
        label_ext(label_item).label_type = LabelType.RASTER
    ...
    def test_label_properties(self):
        ...
        label_prop = label_ext(label_item).label_properties
        self.assertEqual(label_prop, label_item.properties['label:properties'])
        raster_label_prop = label_ext(label_item2).label_properties
        self.assertEqual(raster_label_prop, None)

        # Set
        label_ext(label_item).label_properties = ["prop1", "prop2"]
    ...
    def test_label_classes(self):
        # Get
        label_item = ps.Item.from_file(self.label_example_1_uri)
        label_classes = label_ext(label_item).label_classes
        ...
        label_ext(label_item).label_classes = new_classes
        self.assertEqual([
    ...
    def test_label_tasks(self):
        ...
        label_prop = label_ext(label_item).label_tasks
        self.assertEqual(label_prop, ["classification", "regression"])

        # Set
        label_ext(label_item).label_tasks = ["classification"]
    ...
    def test_label_methods(self):
        ...
        label_prop = label_ext(label_item).label_methods
        self.assertEqual(label_prop, ["manual"])

        # Set
        label_ext(label_item).label_methods = ["manual", "automated"]
    ...
    def test_label_overviews(self):
        # Get
        label_item = ps.Item.from_file(self.label_example_1_uri)
        label_overviews = get_opt(label_ext(label_item).label_overviews)

        label_item2 = ps.Item.from_file(self.label_example_2_uri)
        label_overviews2 = get_opt(label_ext(label_item2).label_overviews)
        ...
        get_opt(label_ext(label_item).label_overviews)[0].counts[1].count = 18
        ...
        get_opt(label_ext(label_item).label_overviews)[1].statistics[0].name = "avg"
        ...
        label_ext(label_item).label_overviews = new_overviews
