...
@pytest.fixture
def example_uri() -> str:
    return get_data_file("projection/example-landsat8.json")
...
def example_summaries_uri() -> str:
    return get_data_file("projection/collection-with-summaries.json")
...
def test_optional_epsg() -> None:
    example_uri = get_data_file("projection/optional-epsg.json")
...
def test_migrate_item_on_1_2() -> None:
    ...
    path = get_data_file("projection/example-with-version-1.2.json")
...
def test_migrate_item_on_1_1() -> None:
    ...
    path = get_data_file("projection/example-with-version-1.1.json")
    item = Item.from_file(path)
...
def test_migrate_collection_item_assets() -> None:
    ...
    path = get_data_file("projection/collection-with-summaries.json")
...
def test_v1_from_dict() -> None:
    with open(get_data_file("projection/example-with-version-1.1.json")) as f:
        data = json.load(f)
...
def test_v1_crs_string() -> None:
    with open(get_data_file("projection/another-1.1.json")) as f:
        data = json.load(f)
    item = pystac.Item.from_dict(data, migrate=False)
    assert item.ext.proj.epsg is not None
