...
def _validate_sources(
    ...
    for i, source in enumerate(v):
        if "name" not in source or "code" not in source:
            raise ValueError(
                f"{SOURCES_PROP}[{i}] must include required keys 'name' and 'code'. "
                f"Got: {source}"
            )
        if not isinstance(source["name"], str) or not source["name"]:
            raise ValueError(f"{SOURCES_PROP}[{i}].name must be a non-empty string.")
        if not isinstance(source["code"], str) or not source["code"]:
            raise ValueError(f"{SOURCES_PROP}[{i}].code must be a non-empty string.")
        if "catalog" in source and (
            not isinstance(source["catalog"], str) or not source["catalog"]
        ):
            raise ValueError(
...
class EarthquakeExtension(
    ...
    def depth(self, v: float | None) -> None:
        self._set_property(
            DEPTH_PROP, None if v is None else float(v), pop_if_none=True
        )
