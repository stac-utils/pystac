...
@pytest.fixture
def example_uri() -> str:
    return str(DATA_FILES / "example-laz.json")
...
def pc_no_stats_item() -> pystac.Item:
    return pystac.Item.from_file(str(DATA_FILES / "example-laz-no-statistics.json"))


@pytest.fixture
def plain_item() -> pystac.Item:
    return pystac.Item.from_file(str(DATA_FILES / "item/sample-item.json"))
...
def test_apply(plain_item: pystac.Item) -> None:
    assert not PointcloudExtension.has_extension(plain_item)

    PointcloudExtension.add_to(plain_item)
    PointcloudExtension.ext(plain_item).apply(
        1000,
