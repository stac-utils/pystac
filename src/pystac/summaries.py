            ...
            if k in self.summaryfields:
                ...
                if strategy in [
                    ...
                    if isinstance(summary, RangeSummary):
                        summary.update_with_value(v)  # pyright: ignore[reportArgumentType]
                    else:
                        summaries[k] = RangeSummary(minimum=v, maximum=v)  # pyright: ignore[reportArgumentType]
                elif strategy in [
...
class Summaries(MutableMapping[str, Any]):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        for k, v in self._store.items():
            if isinstance(v, list) and len(cast(list[Any], v)) >= self.maxcount:
                continue
            out[k] = v.to_dict() if isinstance(v, RangeSummary) else v
        return out
